...
void unlnk(char *cpn)
{
	...
	if ( resultaatnamei==0 || resultaatnamei ==-1) // namei mislukt dus return
	{
		u.u_error=GEENINO;
		printf("resultaat van namei failed failed: %d \n", resultaatnamei);
		return;
	}
	...
	if(Leeg) // leeg maken indien het kan dooe verlagen link counts
	{
		...
		printf("leesblok u.u_diract/BLOKSIZE= %d \n", (u.u_diract/BLOKSIZE));
		LeesBlok(p.i_blok[u.u_diract/BLOKSIZE],buf);
		if(Mapgevonden)
		{
			printf("parent link is %d \n",p.i_link);
			p.i_link--;
			printf("parent link is %d \n",p.i_link);
		...
		printf("dp is %d gestegen \n", (u.u_diract/BLOKSIZE)/DIRLEN);
		dp->d_ino=0;
